...
function Upload() {
 ...
 <div className='w-[90%] max-w-[600px] h-[80px] bg-[white] rounded-full flex justify-around items-center gap-[10px]' onClick={()=>mediaInput.current.click()}>
  
    <div className={`${uploadType=='post'?'bg-black  text-white shadow-2xl shadow-black': ''}
    w-[28%] h-[80%] flex justify-center items-center text-[19px] font-semibold hover:bg-black rounded-full hover:text-white cursor-pointer hover:shadow-2xl hover:shadow-black`} onClick={()=>setUploadType('post')}>Post</div>

    <div className={`${uploadType=='story'?'bg-black  text-white shadow-2xl shadow-black': ''}
    w-[28%] h-[80%] flex justify-center items-center text-[19px] font-semibold hover:bg-black rounded-full hover:text-white cursor-pointer hover:shadow-2xl hover:shadow-black`} onClick={()=>setUploadType('story')}>Story</div>

    <div className={`${uploadType=='loop'?'bg-black  text-white shadow-2xl shadow-black': ''}
    w-[28%] h-[80%] flex justify-center items-center text-[19px] font-semibold hover:bg-black rounded-full hover:text-white cursor-pointer hover:shadow-2xl hover:shadow-black`} onClick={()=>setUploadType('loop')}>Loop</div>
 ...
 {mediaType == 'image' && <div className='w-[80%] max-w-[500px] h-[250px] flex flex-col items-center justify-center mt-[5vh]'>
  ...
  {frontendMedia && <button className='px-[10px] w-[60%] max-w-[400px] h-[50px] bg-[white] mt-[50px] cursor-pointer rounded-2xl'onClick={handleUpload}>{loading?<ClipLoader size={30} color='black'/>:`Upload ${uploadType}`}</button>}
