...
function StoryDp({ ProfileImage, userName, story }) {
  ...
  useEffect(()=>{
    if (userName === 'Your Story') {
      setViewed(true)
    } else if(story?.viewers?.some((viewer)=>
  ...
  },[story,userData,storyData,storyList, userName])
    ...
    <div className="flex flex-col w-[80px] " >
      <div className={`w-[80px] h-[80px] ${!story?null:!viewed?'bg-gradient-to-b from-blue-500 to-blue-950 ': 'bg-gradient-to-r from-gray-500 to-black-800'} rounded-full flex justify-center items-center relative ` } onClick={handleClick}>
        <div className="w-[70px] h-[70px] border-2 border-black rounded-full cursor-pointer overflow-hidden" >
          ...
          {!story && userName == 'Your Story' && (<FiPlusCircle className='text-black absolute bottom-[8px] bg-white right-[10px] rounded-full w-[22px] h-[22px]'/>)}
